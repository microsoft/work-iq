...
8. **Keep personal contacts policy failures honest.** `/me/contacts` is commonly tenant-denied. If WorkIQ returns `Access denied for path: /me/contacts...`, do not retry, fall back to directory writes, or call `workiq-ask`; report that contacts are policy-denied.
