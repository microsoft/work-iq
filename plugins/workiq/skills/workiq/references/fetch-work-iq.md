...
options; [SharePoint](sharepoint-work-iq.md) includes deliberate `$expand=root`
reads without `$select`. Do not probe unsupported variants after a rejection.
...
- [Library metadata](sharepoint-library-metadata.md): authoritative fields,
  column identity, completeness, and per-result status.
- [Business Applications](business-applications.md): discovery and exact returned
  application paths/privileges.
