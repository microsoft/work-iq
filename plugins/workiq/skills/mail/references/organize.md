...
| Create category | `workiq-create_entity` `/me/outlook/masterCategories` using fields returned by schema, commonly display name and color |
| Rename/update category | resolve ID, `workiq-update_entity` `/me/outlook/masterCategories/{id}` |
| Delete category | resolve ID, `workiq-delete_entity` `/me/outlook/masterCategories/{id}` |

Writes on `/me/outlook/masterCategories` are commonly tenant-policy denied. Attempt once after confirmation; if denied, report the exact path and do not route around with `ask`.
