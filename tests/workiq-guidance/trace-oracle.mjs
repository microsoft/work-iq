...
import { packages, broaderCapabilities, retrievalProblems, terminals } from './contract.mjs';
...
export function validateTrace(scenario, trace, { observed = false } = {}) {
  ...
  if (observed) {
    ...
    if (!packages.includes(p?.package)) fail('provenance', 'Unsupported package; this contract covers workiq-preview only.');
